...
sample.metadata <- read_delim(snakemake@input[["metadata"]])

# Get time windows
# TODO: what to do with long tails
...
# TODO: manage edge case for same dates
stopifnot(min.date != max.date)
...
# TODO: clearer checkpoint (at least as many context as our dataset)
stopifnot(nrow(metadata) > nrow(sample.metadata))
